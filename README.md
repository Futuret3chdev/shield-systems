# Futuret3ch Security — Shield Systems

**Defensive visibility only.**

Public materials live in **[public/](public/)**:

| File | Description |
|------|-------------|
| [public/DEVELOPER_GUIDE.md](public/DEVELOPER_GUIDE.md) | Developer guide · `/shield/api` · `/developers` · Games SDK |
| [public/PUBLIC_SHIELD_POLICY.md](public/PUBLIC_SHIELD_POLICY.md) | Acceptable use |
| [public/PRIVACY_NOTE_SHIELD.md](public/PRIVACY_NOTE_SHIELD.md) | Short privacy note |
| [public/shield_systems_diagram.svg](public/shield_systems_diagram.svg) | Endpoint · Edge · Data diagram |
| [public/diagrams/shield_swarm_dashboard.svg](public/diagrams/shield_swarm_dashboard.svg) | Shield Swarm Core dashboard mock (multi-agent style) |
| [PUBLIC_OVERVIEW.md](PUBLIC_OVERVIEW.md) | High-level overview |

**Live product**

- https://memetorrent.futuret3ch.com.au/shield  
- https://memetorrent.futuret3ch.com.au/shield/api  
- https://memetorrent.futuret3ch.com.au/developers  
- Policies: https://www.futuret3ch.com.au/policies.html  

No watermarks on these documents.  
© Futuret3ch Security · MemeTorrent ecosystem
